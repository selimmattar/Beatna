...
class storageFragment : Fragment() {
    val storagelist = ArrayList<SongFile>()
    ...
    fun getMusic(){
        ...
        if(songCursor!=null && songCursor.moveToFirst()){
            ...
            do {
                ...
                songFile= SongFile(currentTitle,currentPath)
                storagelist.add(songFile)
            }while (songCursor.moveToNext())
        }
    }
    fun doStuff(){
        val storage_lv=view?.findViewById<ListView>(R.id.storage_lv)
        val fileAdapter=FileAdapter(context!!, R.id.storage_lv, storagelist)
getMusic()
